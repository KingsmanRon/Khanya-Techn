      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="mtp-visual-edit" -c user.email="noreply@mtp.local" add "${targetFile}"`);
            execSync(`git -c user.name="mtp-visual-edit" -c user.email="noreply@mtp.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
